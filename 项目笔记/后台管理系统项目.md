...
选择：`Choose Vue version、Babel、TypeScript、CSS Pre-processors、Linter / Formatter`
...
创建service文件夹，并创建index.ts作为对外的统一出口。

在service文件夹下创建request文件夹，用来封装axios,在该文件夹下创建index.ts
...
// axios已定义好的ts类型
import type { AxiosInstance, AxiosRequestConfig } from 'axios'
import type { LMRequestInterceptors, LMRequestConfig } from './type'
...
export interface LMRequestInterceptors {
  //定义拦截器类型
  requestInterceptor?: (config: AxiosRequestConfig) => AxiosRequestConfig //请求拦截函数类型
  requestInterceptorCatch?: (error: any) => any //请求失败
  responseInterceptor?: (res: AxiosResponse) => AxiosResponse //响应拦截
  responseInterceptorCatch?: (error: any) => any //响应失败
}

export interface LMRequestConfig extends AxiosRequestConfig {
  interceptors?: LMRequestInterceptors
}
```
...
2、点击立即登录的按钮在`login-panel.vue`中,所以需要从该组件中子组件中的loginAction，当点击发生时，执行loginAction
...
思考问题：vux中的数据都是存在内存中的，登录进去了首页，点击刷新时，token、userInfo、userMenus都想被清除。怎么解决这个问题
