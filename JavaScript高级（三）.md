...
问题：但是如果用户每输入一个单词，都触发一次处理程序去联想可能的内容。假设用户输入：`hellowoeld`，就触发了10次处理程序，十分消耗性能
...
function deepClone(originValue) {
    ...
    for(const key in originValue) {
        //往新对象里边进行添加
        newObj[key] = deepClone(originValue[key]) //递归调用
    }
    return newObj
}
```
...
function deepClone(originValue) {
    ...
    //判断是否基本数据类型
    const isObject = originValue !== null && (typeof originValue === 'object' || typeof originValue === 'function')
    if(!isObject) {
        return originValue
    }
    // 判断是否是set结构
    if(originValue instanceof Set) {
        return new Set([...origniValue])
    }
    //判断是否map结构
...
const obj = {
  ...
  set: new Set(["hello", 123, true]),
  map: new Map([["aaa", "213"], ["bbb", "2321"]])
}
