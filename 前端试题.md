...
思路：创建一个队列Q和一个双端队列D。每次添加值时进行判断，是否添加值大于D的队尾值，大于则删除队尾值直到小于队尾值，插入队尾。这样以来，D就维持了单调递减的规律。
...
> 设置两个指针p1和p2，代表当前正在处理中的nums1或nums2的位置。每次应用后减1
>
> 以及当前正在处理的元素位置tail，每次成功处理后减1
>
> 遍历比较两者大小，大的数往tail所代表的nums的位置存放
