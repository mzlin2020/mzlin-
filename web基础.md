...
HTTP协议是无状态的协议。一旦数据交换完毕，客户端与服务器端的连接就会关闭，再次交换数据需要建立新的连接。服务器无法从连接上跟踪会话
...
1、输入`www.bilibili.com`网址后，首先在本地的域名服务器中查找，没找到就去根域名服务器查找，没找到再去`com`顶级域名服务器查找。如此类推下去，直到找到IP地址。

大致过程：

`. —— .com . —— bilibili.com. —— www.bilibili.com.`

(最开始的.对应的是根域名服务器，默认情况下所有的网址的最后一位都是.，所以为了方便用户，通常都会省略，浏览器在请求DNS的时候会自动加上)

2、找到IP地址后，本地DNS服务器会把IP地址返回给用户电脑，还会把这个对应关系保存在缓存中，以备下次别的用户查询时，可以直接返回结果，加快网络访问。
